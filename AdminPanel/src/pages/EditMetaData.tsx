...
const EditMetaData = () => {
  ...
  const getMetaData = async () => {
    try {
      const res = await axiosInstance.get(`/getsinglemetadata/${id}`);
      if (res.status === 200) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await axiosInstance.put(`/updatemetadata/${id}`, data);
      if (res.status === 200) {
        toast.success(res.data.message || 'Metadata updated successfully');
        navigate('/metadata');
      }
    } catch (error) {
      if (isAxiosError(error)) {
