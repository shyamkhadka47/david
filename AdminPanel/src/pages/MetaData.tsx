...
const MetaData = () => {
  ...
  const getAllMetaData = async () => {
    try {
      const res = await axiosInstance.get('/getallmetadata');
      if (res.status === 200) {
  ...
  const handleDelete = async (metaId: string) => {
    try {
      const res = await axiosInstance.delete(`/deletemetadata/${metaId}`);
      if (res.status === 200) {
